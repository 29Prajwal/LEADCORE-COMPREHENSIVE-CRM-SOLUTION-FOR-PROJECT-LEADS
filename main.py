...
def Admindashboard():
    if request.method == 'POST':
        ...
        if email == 'admin@gmail.com' and password == 'prajwal':
            session['email'] = email
...
def AssignProject():
    if request.method == 'POST':
        ...
        if selected_employee_ids and selected_project:
            ...
            try:
                ...
                query = f"SELECT emp_id, emp_name FROM employee WHERE emp_id IN ({format_strings})"
                cur.execute(query, tuple(selected_employee_ids))
                ...
                print("REAL DB ERROR:", e)   # ← now you will see actual error in terminal
    ...
    cur.execute("""
        ...
        WHERE emp_id NOT IN (SELECT employee_id FROM assign_project)
    """)
    result = cur.fetchall()
    ...
    return render_template("admin/assign_project.html", result=result, project=project)

#-------Show Assign Project Page-----latest code----
@app.route('/show_assign_project', methods=['GET', 'POST'])
...
def client_register():
    ...
    if request.method == 'POST':
        ...
        # Insert user
        cur.execute("""
            INSERT INTO client (lead_name,email,mobile,category,password)
            VALUES (%s,%s,%s,%s,%s)
        """,(name,email,mobile,category,hashed_password))
    ...
    return render_template("client/client_registration.html")
    ...
    if request.method == "POST":
        ...
        if user:
            if check_password_hash(user['password'], password):
                ...
                session['client_name'] = user['lead_name']
...
def cdashboard():
    return render_template("client/cdashboard.html")
...
def saving():
    ...
    enquiry_details = request.form["txtenqdetails"]

    cur = mysql.connection.cursor()
    cur.execute(
        'INSERT INTO enquiry(name,email,category,details) VALUES(%s,%s,%s,%s)',
        (name,email,enqsubject,enquiry_details)
    )
    mysql.connection.commit()
    cur.close()
...
def show_enquiry():
    ...
    cur.execute("SELECT * FROM enquiry ORDER BY id DESC")
    result = cur.fetchall()
    cur.close()
...
def update_profile():
    ...
    id = session['client_id']   # login client id
    ...
    if request.method == "POST":
        ...
        cur.execute("""
            UPDATE client
            SET lead_name=%s, email=%s, mobile=%s, password=%s
            WHERE id=%s
        """, (lead_name, email, mobile, password, id))
    ...
    return render_template("client/update_profile.html", user=user)
from flask import flash, redirect, url_for

@app.route('/client_payment', methods=['GET','POST'])
def client_payment():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("""
        INSERT INTO client_payment(client_name,email,amount,payment_method,transaction_id)
        VALUES(%s,%s,%s,%s,%s)
        """,(client_name,email,amount,payment_method,transaction_id))
        ...
        # page reload
        return redirect(url_for('client_payment'))

    return render_template("client/client_payment.html",success=True)
...
def client_view_payment():
    ...
    return render_template("client/client_view_payment.html", payments=payments)
@app.route('/client_payment_delete')
def client_payment_delete():
    ...
    return redirect(url_for('client_view_payment'))
